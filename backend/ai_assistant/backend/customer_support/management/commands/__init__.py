...
class Command(BaseCommand):
    ...
    def crear_tickets(self, clientes):
        ...
        tickets_data = [
            ...
            {
                'titulo': 'Consulta sobre descuentos disponibles',
                'descripcion': 'Me gustaría saber qué descuentos están disponibles para clientes frecuentes.',
                'estado': 'resuelto',
                'prioridad': 'baja',
            },
            {
                'titulo': 'Problema técnico con la app móvil',
                'descripcion': 'La aplicación móvil se cierra automáticamente al intentar consultar mi saldo.',
                'estado': 'en_proceso',
                'prioridad': 'media',
            },
            {
                'titulo': 'Reembolso de cobro duplicado',
                'descripcion': 'Se me cobró dos veces el mismo servicio. Necesito que me reembolsen una transacción.',
                'estado': 'abierto',
                'prioridad': 'critica',
            },
            {
                'titulo': 'Configuración de notificaciones',
                'descripcion': 'Quiero activar las notificaciones por email para mis pagos y vencimientos.',
                'estado': 'cerrado',
                'prioridad': 'baja',
            },
        ]
        ...
        for i, data in enumerate(tickets_data):
            ...
            # Si está resuelto o cerrado, agregar fecha de resolución
    ...
    def crear_pagos(self, clientes):
        ...
        for cliente in clientes:
            num_pagos = random.randint(1, 8)  # 1-8 pagos por cliente
            
            for _ in range(num_pagos):
                # Monto aleatorio
                monto = Decimal(str(round(random.uniform(50, 1500), 2)))
                ...
                # IMPORTANTE: Primero crear el pago SIN actualizar el saldo
                # para evitar duplicar el incremento
                pago = Pago(
                    cliente=cliente,
                    monto=monto,
                ...
                # Guardar SIN triggear el save automático que actualiza saldo
    ...
    def crear_historial(self):
        ...
        acciones_data = [
            ...
            {
                'tipo': 'ai_tool',
                'descripcion': 'AI Tool: Pago registrado $500.00 - Juan Carlos Pérez',
                'metadata': {'pago_id': 1, 'monto': 500.00}
            },
        ]
